...
static bool _initGpu(void) {
  ...
  C3D_RenderTargetSetOutput(topScreen, GFX_TOP, GFX_LEFT,
  ...
  upscaleBuffer =
      C3D_RenderTargetCreateFromTex(&upscaleBufferTex, GPU_TEXFACE_2D, 0, 0);
  if (!upscaleBuffer) {
    return false;
  }
...
static void _guiFinish(void) { ctrFlushBatch(); }
...
static void _setup(struct mGUIRunner *runner) {
  ...
  if (mCoreConfigGetUIntValue(&runner->config, "darkenMode", &mode) &&
      mode < DM_MAX) {
    darkenMode = mode;
  }
  frameLimiter = true;
...
static void _gameLoaded(struct mGUIRunner *runner) {
  ...
  int fakeBool;
  if (mCoreConfigGetIntValue(&runner->config, "interframeBlending",
                             &fakeBool)) {
...
static void _drawScreenshot(struct mGUIRunner *runner, const mColor *pixels,
  ...
  unsigned y;
  for (y = 0; y < height; ++y) {
    memcpy(&screenshotBuffer[y * 256], &pixels[y * width],
           width * sizeof(mColor));
    memset(&screenshotBuffer[y * 256 + width], 0,
           (256 - width) * sizeof(mColor));
  }

  GSPGPU_FlushDataCache(screenshotBuffer, 256 * height * sizeof(mColor));
  C3D_SyncDisplayTransfer((u32 *)screenshotBuffer, GX_BUFFER_DIM(256, height),
                          tex->data, GX_BUFFER_DIM(256, 256),
                          GX_TRANSFER_IN_FORMAT(GX_TRANSFER_FMT_RGB565) |
                              GX_TRANSFER_OUT_FORMAT(GX_TRANSFER_FMT_RGB565) |
...
static uint16_t _pollGameInput(struct mGUIRunner *runner) {
  ...
  uint16_t keys =
      mInputMapKeyBits(&runner->core->inputMap, _3DS_INPUT, activeKeys, 0);
...
static void _startRequestImage(struct mImageSource *source, unsigned w,
                               unsigned h, int colorFormats) {
  UNUSED(colorFormats);
  ...
  if (imageSource->cam) {
    CAMU_SetReceiving(&imageSource->handles[0], imageSource->buffer, PORT_CAM1,
                      imageSource->bufferSize, imageSource->transferSize);
  }
}
...
static void _requestImage(struct mImageSource *source, const void **buffer,
                          size_t *stride, enum mColorFormat *colorFormat) {
  struct m3DSImageSource *imageSource = (struct m3DSImageSource *)source;
...
static void _postAudioBuffer(struct mAVStream *stream,
                             struct mAudioBuffer *buffer) {
  UNUSED(stream);
  if (hasSound == DSP_SUPPORTED) {
    int startId = bufferId;
    while (dspBuffer[bufferId].status == NDSP_WBUF_QUEUED ||
           dspBuffer[bufferId].status == NDSP_WBUF_PLAYING) {
    ...
    DSP_FlushDataCache(dspBuffer[bufferId].data_pcm16,
                       AUDIO_SAMPLES * 2 * sizeof(int16_t));
...
static enum GUIKeyboardStatus _keyboardRun(struct GUIKeyboardParams *keyboard) {
  ...
  SwkbdButton button =
      swkbdInputText(&swkbd, keyboard->result, sizeof(keyboard->result));
...
THREAD_ENTRY _core2Test(void *context) { UNUSED(context); }
...
int main(int argc, char *argv[]) {
  ...
  if (!ndspInit()) {
    ...
    audioLeft =
        linearMemAlign(AUDIO_SAMPLES * DSP_BUFFERS * 2 * sizeof(int16_t), 0x80);
  ...
  for (i = 0; i < 2; ++i) {
    ...
    // Zero texture data to make sure no garbage around the border interferes
    // with filtering
    GX_MemoryFill(outputTexture[i].data, 0x0000, outputTextureEnd,
                  GX_FILL_16BIT_DEPTH | GX_FILL_TRIGGER, NULL, 0, NULL, 0);
    gspWaitForPSC0();
  }
  ...
  struct mGUIRunner runner = {
      ...
      .keySources = (struct GUIInputKeys[]){{.name = "3DS Input",
                                             .id = _3DS_INPUT,
                                             .keyNames =
                                                 (const char *[]){
                                                     "A",
                                            ...
                                            {.id = 0}},
      .configExtra =
          (struct GUIMenuItem[]){{.title = "Screen mode",
                                 ...
                                 {.title = "Filtering",
                                  ...
                                  .validStates =
                                      (const char *[]){
                                          NULL, // Disable choosing nearest
                                                // neighbor; it always looks bad
                                          "Bilinear (smoother)",
                                          "Bilinear (pixelated)",
                                          "Bilinear (ultrasharp)",
                                      },
  ...
  runner.autosave.thread =
      threadCreate(mGUIAutosaveThread, &runner.autosave, 0x4000, 0x1F, 1, true);
  ...
  _map3DSKey(&runner.params.keyMap, KEY_CSTICK_UP,
             mGUI_INPUT_INCREASE_BRIGHTNESS);
  _map3DSKey(&runner.params.keyMap, KEY_CSTICK_DOWN,
